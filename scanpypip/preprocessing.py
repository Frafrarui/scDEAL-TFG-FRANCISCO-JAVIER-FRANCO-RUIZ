...
def read_sc_file(file_path,header=0,index_col=0,sep=None):
    ...
    # deal with csv file  si es csv o txt usamos panda.read_csv
    ...
    # deal with 10x h5 file si es h5ad o h5 utiliza distinto lectores
    ...
    return gene_expression #convierte el dataframe en anndata
...
def concat(adata_dict,join='inner', sample_key='sample', batch_categories=None, index_unique='-', combat = False, combat_key = "batch",covariates=None):
    '''
    util para concatenar varios adata devolviendo uno solo
...
def cal_ncount_ngenes(adata,sparse=False,remove_keys=[]):
    ...
    mito_genes = (adata.var_names.str.lower().str.rfind('mt-'))!=-1 #almacena genes mitocondriales
    rps_genes = (adata.var_names.str.lower().str.rfind('rps'))!=-1 #almacena genes ribosoma s
    rpl_genes = (adata.var_names.str.lower().str.rfind('rpl'))!=-1 #almacena genes ribosoma l
    ...
    # Una vez clasificado los genes lo que hacemos es calcular
    # metricas con ellos y anadimos columnas nuevas en adata.obs
    # como n_genes_by_counts, total_counts , pct_counts_mt-,
    # pct_counts_rps, pct_count_rpl(porentaje de genes ribosoma L)
    ...
    if len(remove_keys)>0:
        ...
        adata = adata[:, mask]
#SIrve para eliminar genes de un tipo por ejemplo si le paso
#adata=cal_ncount_ngenes(data , remove_keys=[¨mt-¨, ¨rps¨] eso lo q hace es 
#eliminar de dataset todos los genes mitocondriales y ribosomicos
...
def receipe_my(adata,l_n_genes = 500, r_n_genes= 5000, filter_mincells=3,filter_mingenes=200, percent_mito = 5, normalize = False,log = False,sparse = False,plotinfo= False,
                remove_genes=[]):
#ESTA FUNCION ES COMO UN PIPELINE
