...
#Esta clase define un autoencoder clasico, transforma los dato de entrada a una representacion
#latente 
class AEBase(nn.Module):
    def __init__(self,
                 input_dim,#numero de genes entrada
                 latent_dim=128,
                 h_dims=[512],
                 drop_out=0.3):
        ...
        # Build Encoder, construimos el encoder AE 
        ...
        # Build Decoder, el decoder intenta reconstruir los datos originakes a partir de z
        modules = []
        ...
        #     self.bottleneck
        # )            
    #EL decoder hace lo mismo que el encoder pero en orden inverso 


             
    def encode(self, input: Tensor):
        """
        Toma un input y devuelve el vector latente z
        """
    ...
    def decode(self, z: Tensor):
        """
        Toma el vector latente z y devuelve una reconstruccion del input 
        """
    ...
    def forward(self, input: Tensor, **kwargs):
        ...
        return  output   
    #El forwar es la llamada estandar de Pytorch que realiza:
        #1. Codificacion del input a z
        #2.Decodificacion de z para reconstruir la entrada 

# Model of Predictor, es una red neuronal que toma como entrada un vector latente z
#y produce una probavbilidad de q sea sensible o resitente 
class Predictor(nn.Module):
    def __init__(self,
                 input_dim,
                 output_dim=1,
                 h_dims=[512],#numero de capas ocultas, eso significa 1 capa con 512 neuronas
                 drop_out=0.3):
    ...
    def forward(self, input: Tensor, **kwargs):
        ...
        return  output
    #Tomo un vector de entrada , lo paso por las capas ocultas, luego por la salida y devuelvo una probabilidad
        
        
    
# Model of Pretrained P
class PretrainedPredictor(AEBase):#La entrada es un autoencoder , solo le imtersa la parte del encoder , no reconstruirlo
    def __init__(self,
        ...
        # Only extract encoder, como solo nos interesa el encoder, borra todo lo relacionado con el decoder
        del self.decoder
    ...
    def predict(self, embedding, **kwargs):
        output = self.predictor(embedding)
        return  output 
    #EN resumen coge la clase AE y obtiene el encoder y la clase Predictor y las junta las dos sin el decoder 
...
class VAEBase(nn.Module):#Es otro autoencoder, pero este da un vector una distribucion
    #promedio (mu) y una desviacion estandar (sigma) luego toma un valor alatorio dentro de esa distribucion y reconstruye la entrada 
    def __init__(self,
                 input_dim,
                 latent_dim=128,
                 h_dims=[512],
...
class PretrainedVAEPredictor(VAEBase):#hace lo mismo que el otro pretrained pero la diferencia esq el encoder es VAE
    def __init__(self,
                 # Params from AE model
                 input_dim,
                 latent_dim=128,
...
#Dann es una clase que se usa pra trasnferir el conocimiento de bulk a single cell
class DaNN(nn.Module): 
    def __init__(self, source_model,target_model,fix_source=False):
                        #Como vemos recibe de entrada un modelo preentrenado con datos bulk (PretrainedPredictor)(encoder +predictor)
                        #Un nuevo encoder que queremos adaptar a single cell , solo recibe el encoder ,, no tiene el predictor toadvia ( lo aprende gracvias al source)
    ...
    def forward(self, X_source, X_target,C_target=None):
     #  entrada      datos bulk datos sc etiquetas para sc
        x_src_mmd = self.source_model.encode(X_source)#TOma los datos de bulk y los pasa por el encoder del modelo source 

        if(type(C_target)==type(None)): # si no hay etiquetas entonces se codifican 
            x_tar_mmd = self.target_model.encode(X_target)
        else:
            x_tar_mmd = self.target_model.encode(X_target,C_target)

        y_src = self.source_model.predictor(x_src_mmd)#tomamos el x de bulk q es (x_src_mmd) y lo pasamos por el predictor , eso devuelve y_src que es la predcicion para esos datos bulk
        return y_src, x_src_mmd, x_tar_mmd
   

class TargetModel(nn.Module):#Target model se usa despues de entrenar al DANN su funcion es hacer predicciones 
    #directamente en datos de single cell utilizando el encoder target_encoder(encoder entrenado de single cell) y el predictor del modelo bulk (Source_predictor)
...
def g_loss_function(preds, labels, mu, logvar, n_nodes, norm, pos_weight):
    ...
    #Cuando usamos VAE , usamos esta funcion para calcular el error total que comete el modelo
    #Hay dos errores : 
            #1.Error de clasificacion preds= lo que el mnodelo predijo, labels=lo q deberia haber predecido , resultado= cuanto se equivoco al clasificar 
    # Check if the model is simple Graph Auto-encoder
    if logvar is None: #solo se usa cuando usamos VAE
        return cost
    ...
    #KLD es una penalizacion exra para que el vector z que genera el VAE no se vuelva loco y siempre este en un rango razonable 
    KLD = -0.5 / n_nodes * torch.mean(torch.sum(
        1 + 2 * logvar - mu.pow(2) - logvar.exp().pow(2), 1))
    return cost + KLD #Error total del modelo